...
params_est_old = [0.9169, 0.319, 0.3888, -0.265, 0.0505, -0.1198, 0.4968, 0.5799]
# fitted w/ 10 quantiles, chisq = 606
params_est = [0.9452, 0.3236, 0.4126, -0.2745, 0.0486, -0.124, 0.5001, 0.5527]
# fitted w/ 10 quantiles, chisq = 440

params_est = [0.8, 0.3236, 0.1, -0.2745, 0.1, -0.124, 0.5001, 0.5527]
# demo values
...
def plot_schematic(model_params):
    ...
    resp_time = t[cross_idx] - 0.5 * DELTA_T
    ...
    if len(cross_indices):
        cross_idx_n = cross_indices[0]
        # take the first index
    else:  # i.e., if no crossing was found
        cross_idx_n = NR_TSTEPS - 1
        # set it to the final index
    # 4. Now we can use this index to determine both the decision and the response time
    decisions_n = positions_n[cross_idx_n] > 0
    resp_time_n = t[cross_idx_n] - 0.5 * DELTA_T
    # i.e., the midpoint of the crossing interval
    ...
    pl.savefig(filename.replace("png", "svg"), dpi=100, bbox_inches="tight", pad_inches=0, transparent=True)
