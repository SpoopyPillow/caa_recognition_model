...
YAML_FILENAME = "data/neha_data_revised.yml"
...
def save_word_lists():
    ...
    target_words = [el["target"] for el in neha_data if el["judgment"] == "hit" or el["judgment"] == "miss"]

    target_words = np.unique(target_words)

    lure_words = [el["target"] for el in neha_data if el["judgment"] == "FA" or el["judgment"] == "CR"]

    lure_words = np.unique(lure_words)

    target_file = open("data/target_list.txt", "w")
    target_file.write("\n".join(target_words))
    target_file.close()

    lure_file = open("data/lure_list.txt", "w")
    lure_file.write("\n".join(lure_words))
    lure_file.close()
