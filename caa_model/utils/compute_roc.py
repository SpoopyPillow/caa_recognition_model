...
def get_conf_crit(trial, use_raw_conf=True):
    ...
    criterion = neutral_point + adjusted_confidence if detected else neutral_point - adjusted_confidence - 1
...
def get_rt_crit(trial, rt_quantiles, use_normed=True):
    ...
    criterion = neutral_point + rt_rank if detected else neutral_point - rt_rank - 1
...
def computeAndPlotROC(filename=EXPT1_FILENAME, subid=None, use_normed_rts=True):
    ...
    if subid is not None:
        filename += "_s%s" % subid
    pl.savefig(filename + ".png", transparent=True, dpi=300, bbox_inches="tight", pad_inches=0.05)
