...
from ..utils.get_yaml_data import filter_word_data
...
# combined.
FullParams = namedtuple("FullParams", ["c", "mu_old", "d", "mu_new", "tc_bound", "z0", "deltaT", "t_offset"])

params_est_old = FullParams(0.9478, 0.3213, 0.4163, -0.2832, 0.047, -0.1273, 0.4787, 0.5894)
# fitted w/ 10 quantiles, chisq = 601
params_est = FullParams(0.9703, 0.3282, 0.4126, -0.2719, 0.0471, -0.1242, 0.5034, 0.5692)
# fitted w/ 10 quantiles and remember/know categories, chisq = 760

param_bounds = FullParams(
    (0.0, 1.0), (-2.0, 2.0), (EPS, 2.0), (-2.0, 2.0), (0.05, 1.0), (-1.0, 1.0), (EPS, 2.0), (0, 0.5)
)
...
def compute_gof_all(
    model_params, quantiles=NR_QUANTILES, remknow=True, data=DATA, use_chisq=True, use_rk_hack=False
):
    """
    ...
    if remknow:
        ...
        res = compute_model_gof_rk(
            params_est_old, *old_data, nr_quantiles=quantiles, use_chisq=use_chisq, use_rk_hack=use_rk_hack
        ) + compute_model_gof_rk(
            params_est_new, *new_data, nr_quantiles=quantiles, use_chisq=use_chisq, use_rk_hack=use_rk_hack
        )
    else:
        # computes gof using concatenated remember and know responses
        old_data = [
            pl.hstack([data.rem_hit.rt, data.know_hit.rt]),
            data.miss.rt,
            pl.hstack([data.rem_hit.conf, data.know_hit.conf]),
        ]
        new_data = [
            pl.hstack([data.rem_fa.rt, data.know_fa.rt]),
            data.CR.rt,
            pl.hstack([data.rem_fa.conf, data.know_fa.conf]),
        ]
        # compute the combined goodness-of-fit
        res = compute_model_gof(
            params_est_old, *old_data, nr_quantiles=quantiles, use_chisq=use_chisq
        ) + compute_model_gof(params_est_new, *new_data, nr_quantiles=quantiles, use_chisq=use_chisq)
    return res


def compute_prop_all(model_params, quantiles=NR_QUANTILES, remknow=True, data=DATA, use_rk_hack=False):
    """
    computes the overall goodness-of-fit of the model defined by model_params.
    This is the sum of the NLL or chi-square statistics for the distribution
    of responses to both the old and new words.
    ...
    if remknow:
        ...
        new_data = [data.rem_fa.rt, data.know_fa.rt, data.CR.rt, data.rem_fa.conf, data.know_fa.conf]
        # compute the combined goodness-of-fit
        p_obs_old, p_pred_old = compute_model_prop_rk(
            params_est_old, *old_data, nr_quantiles=quantiles, use_rk_hack=use_rk_hack
        )
        p_obs_new, p_pred_new = compute_model_prop_rk(
            params_est_new, *new_data, nr_quantiles=quantiles, use_rk_hack=use_rk_hack
        )
    else:
        # computes gof using concatenated remember and know responses
        old_data = [
...
def compute_model_gof(model_params, old_RTs, new_RTs, old_conf, nr_quantiles, use_chisq=True):
    ...
    old_freqs = pl.array(
        [
            -pl.diff([pl.sum(old_RTs[old_conf == i] > q) for q in old_quantiles[i]] + [0])
...
def compute_model_gof_rk(
    model_params, rem_RTs, know_RTs, new_RTs, rem_conf, know_conf, nr_quantiles, use_chisq=True, use_rk_hack=False
):
    """
    computes the goodness-of-fit of the model defined by model_params to the
    ...
    rem_freqs = pl.array(
        [
            -pl.diff([pl.sum(rem_RTs[rem_conf == i] > q) for q in old_quantiles[i]] + [0])
            for i in range(nr_conf_levels)
        ]
    )
    know_freqs = pl.array(
        [
            -pl.diff([pl.sum(know_RTs[know_conf == i] > q) for q in old_quantiles[i]] + [0])
            for i in range(nr_conf_levels)
        ]
    )
    ## I think this is where the problem was. The confidence levels in the
...
def compute_model_prop_rk(
    model_params, rem_RTs, know_RTs, new_RTs, rem_conf, know_conf, nr_quantiles, use_rk_hack=False
):
    """
    computes the predicted proportions for the model defined by model_params as well as
...
def predicted_proportions(c, mu_f, d_f, tc_bound, z0, deltaT, t_offset=0, use_fftw=True):
    ...
    for j in range(1, len(clims)):
        ...
        p_old_conf[j - 1, to_idx] = (
            p_old[to_idx] * pl.diff(stats.norm.cdf([clims[j], clims[j - 1]], mu_delta, s_delta)).item()
        )
    #######################################
    ...
    for i in range(to_idx + 1, len(t)):
        ...
        for j in range(1, len(clims)):
            p_old_conf[j - 1, i] = (
                p_old[i] * pl.diff(stats.norm.cdf([clims[j], clims[j - 1]], mu_delta, s_delta)).item()
            )
...
def emp_v_prediction(model_params, nr_conf=2, data=DATA, rk=True, rk_hack=False):
    ...
    if rk:
        ...
        if rk_hack:
            ...
            # this is for Arnold's suggested hack
            rc_freqs = (
                pl.array([len(el) for el in hr_rts])[:, None] + pl.array([len(el) for el in fr_rts])[:, None]
            )
            kc_freqs = (
                pl.array([len(el) for el in hk_rts])[:, None] + pl.array([len(el) for el in fk_rts])[:, None]
            )
            total_ec = pl.double(rc_freqs + kc_freqs)
            p_rem_ec = rc_freqs / total_ec
            p_know_ec = 1 - p_rem_ec
...
def plot_comparison(model_params, nr_conf_bounds=2, rk=True, rk_hack=False):
    ...
    old_data, new_data, pp_old, pp_new, n_old, n_new = emp_v_prediction(
        model_params, nr_conf_bounds, rk=rk, rk_hack=rk_hack
    )
    ...
    if rk:
        pl.legend(
            curves,
            ["new", "rem, conf=2", "know, conf=2", "rem, conf=1", "know, conf=1", "rem, conf=0", "know, conf=0"],
            loc="best",
            prop={"size": "small"},
        )
    else:
        pl.legend(curves, ["new", "old, conf=2", "old, conf=1", "old, conf=0"], loc="best", prop={"size": "small"})
