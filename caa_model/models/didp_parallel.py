...
def compute_gof_all(model_params, quantiles=NR_QUANTILES, spm=False, data=DATA, use_chisq=True):
    ...
    if spm:
        ...
        c_r, c_f, mu_r, mu_f, d_r, d_f, tc_bound_r, tc_bound_f, z0_r, z0_f, mu_r0, mu_f0, deltaT, t_offset = (
            model_params
        )
    params_est_old = [
        [c_r, 0],
...
def compute_model_gof(
    ...
    rem_quantiles, know_quantiles, new_quantiles, p_r, p_k, p_n = compute_model_quantiles(
        model_params, nr_quantiles
    )
    ...
    rem_freqs = pl.array(
        [
            -pl.diff([pl.sum(rem_RTs[rem_conf == i] > q) for q in rem_quantiles[i]] + [0])
            for i in range(nr_conf_levels)
        ]
    )
    know_freqs = pl.array(
        [
            -pl.diff([pl.sum(know_RTs[know_conf == i] > q) for q in know_quantiles[i]] + [0])
            for i in range(nr_conf_levels)
        ]
    )
...
def predicted_proportions(
    c_r, c_f, mu_r, mu_f, d_r, d_f, tc_bound_r, tc_bound_f, z0_r, z0_f, deltaT, t_offset=0, use_fftw=USE_FFTW
):
    ...
    for i in range(to_idx, len(t)):
        ...
        for j in range(1, len(clims_r)):
            ...
            p_rem_conf[j - 1, i] = p_old_rem * pl.diff(
                stats.norm.cdf([clims_r[j], clims_r[j - 1]], mu_r_delta, s2_r_delta)
            )
            p_know_conf[j - 1, i] = p_old_know * pl.diff(
                stats.norm.cdf([clims_f[j], clims_f[j - 1]], mu_f_delta, s2_f_delta)
            )
...
def emp_v_prediction(model_params, data=DATA):
    conf_r, conf_f, mu_r, mu_f, d_r, d_f, tc_bound_r, tc_bound_f, z0_r, z0_f, mu_r0, mu_f0, deltaT, t_offset = (
        model_params
    )
    # mu_f0 = mu_r0 =0
    c_r = [conf_r, 0]
