...
class SISPZ0VarRBoundDDM(BaseDDM):
    class Params(NamedTuple):
        ...
        z0: float             # SINGLE shared starting point
        t_post: float         # Post-decision accumulation time
        t0: float             # Non-decision time

    @staticmethod
    def split_params(model_params):
        # Unpack exactly 10 parameters
        c_val, mu_t, mu_l, d, tc, r_off, s_r_off, z0, dT, t0 = model_params
        c_list = [c_val, 0]

        # Feed the exact same z0 to both targets and lures
        target_params = (c_list, mu_t, d, tc, r_off, s_r_off, z0, dT, t0)
        lure_params = (c_list, mu_l, d, tc, r_off, s_r_off, z0, dT, t0)
