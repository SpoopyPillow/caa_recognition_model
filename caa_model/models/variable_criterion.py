...
def format_as_simulation(data):
    rts = (
        pl.concatenate(
            (data.rem_hit.rt, data.know_hit.rt, data.rem_fa.rt, data.know_fa.rt, data.CR.rt, data.miss.rt)
        )
        * 1000
    )
    confs = pl.concatenate(
    ...
    is_old[: (rh_l + kh_l + rf_l + kf_l)] = (
        True  # Set remember hit, know hit, remember FA, know FA = True in is_old
    )
...
def vc_model_NLL(data, params):
    ...
    s_a = pl.zeros(pl.shape(s))
    s_a[:] = -pl.inf
    s_b = pl.zeros(pl.shape(s))
    s_b[:] = pl.inf
    # ... for 'new' responses
    ...
    rk_LLs[is_remember] = tnorm_logcdf(s[is_remember], s_a[is_remember], s_b[is_remember], crit_mu, crit_sd)
    ## 'old-know': p(crit_r > s |old,conf)
    is_know = pl.logical_not(is_remember)
    rk_LLs[is_know] = tnorm_logsf(s[is_know], s_a[is_know], s_b[is_know], crit_mu, crit_sd)
...
def vcvb_model_NLL(data, params):
    ...
    ## conf=2
    conf_LLs[array_and(is_old, confs == 2)] = stats.norm.logcdf(
        s[array_and(is_old, confs == 2)], conf1_mu + conf2_delta, conf_sd
    )
    ## conf=1
    conf_LLs[array_and(is_old, confs == 1)] = stats.norm.logcdf(
        s[array_and(is_old, confs == 1)], conf1_mu, conf_sd
