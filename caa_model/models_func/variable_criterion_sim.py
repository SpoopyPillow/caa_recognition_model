...
DPMParams = namedtuple(
    "DPMParams", ["T_MU", "T_SD", "CRIT_O", "CRIT_MU", "CRIT_SD", "E_A", "E_B", "E_C", "CONF1", "CONF2"]
)
param_bounds = DPMParams(
    (-2.0, 2.0),
    (EPS, 2.0),
    (-2.0, 2.0),
    (-2.0, 2.0),
...
rts = (
    np.concatenate((data.rem_hit.rt, data.know_hit.rt, data.rem_fa.rt, data.know_fa.rt, data.CR.rt, data.miss.rt))
    * 1000
)
confs = np.concatenate(
    (data.rem_hit.conf, data.know_hit.conf, data.rem_fa.conf, data.know_fa.conf, data.CR.conf, data.miss.conf)
)
...
low_confs2 = confs2 == 0
rh_high2 = (
    rts2[pl.logical_and(pl.logical_and(pl.logical_and(is_target2, is_old2), is_remember2), high_confs2)] * 0.001
)  # target - remember hit with high confidence
rh_med2 = (
    rts2[pl.logical_and(pl.logical_and(pl.logical_and(is_target2, is_old2), is_remember2), med_confs2)] * 0.001
)  # target - remember hit with med confidence
rh_low2 = (
    rts2[pl.logical_and(pl.logical_and(pl.logical_and(is_target2, is_old2), is_remember2), low_confs2)] * 0.001
)  # target - remember hit with low confidence

kh_high2 = (
    rts2[
        pl.logical_and(
            pl.logical_and(pl.logical_and(is_target2, is_old2), pl.logical_not(is_remember2)), high_confs2
        )
    ]
    * 0.001
)  # target - know hit with high confidence
kh_med2 = (
    rts2[
        pl.logical_and(
            pl.logical_and(pl.logical_and(is_target2, is_old2), pl.logical_not(is_remember2)), med_confs2
        )
    ]
    * 0.001
)  # target - know hit with med confidence
kh_low2 = (
    rts2[
        pl.logical_and(
            pl.logical_and(pl.logical_and(is_target2, is_old2), pl.logical_not(is_remember2)), low_confs2
        )
    ]
    * 0.001
)  # target - know hit with low confidence
...
kh_high = (
    rts[pl.logical_and(pl.logical_and(pl.logical_and(is_target, is_old), np.logical_not(is_remember)), high_confs)]
    * 0.001
)
kh_med = (
    rts[pl.logical_and(pl.logical_and(pl.logical_and(is_target, is_old), np.logical_not(is_remember)), med_confs)]
    * 0.001
)
kh_low = (
    rts[pl.logical_and(pl.logical_and(pl.logical_and(is_target, is_old), np.logical_not(is_remember)), low_confs)]
    * 0.001
)
